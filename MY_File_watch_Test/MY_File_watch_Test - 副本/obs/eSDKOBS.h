...
typedef enum
{
    ...
    /**
    * Errors from the obs service 来自obs服务的错误
    **/
	...
	*下面是obs返回的HTTP错误，没有足够的细节来区分上述obs_状态的错误条件
    */
    ...
    /*
    * posix new add errors posix新添加错误
    */
     OBS_STATUS_QuotaTooSmall,

    /*
    * obs-meta errors obs元错误
    */
...
typedef enum
{
    OBS_STORAGE_CLASS_STANDARD              = 0, /* STANDARD 标准*/
    OBS_STORAGE_CLASS_STANDARD_IA           = 1, /* STANDARD_IA 标准_IA*/
    OBS_STORAGE_CLASS_GLACIER               = 2, /* GLACIER  网络*/
...
typedef enum
{
    OBS_CANNED_ACL_PRIVATE                     = 0,  //used by s3 and obs api 使用S3和OBS API
    OBS_CANNED_ACL_PUBLIC_READ                 = 1,  //used by s3 and obs api
    OBS_CANNED_ACL_PUBLIC_READ_WRITE           = 2,  //used by s3 and obs api
    OBS_CANNED_ACL_AUTHENTICATED_READ          = 3,  //only used by s3 api 仅使用s3 api
    ...
    OBS_CANNED_ACL_PUBLIC_READ_DELIVERED       = 7,  //only used by obs api 仅使用obs api
...
// only object bucket can use 只有对象存储桶可以使用
